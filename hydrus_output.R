...
read_hydrus.out<-function(treat=17,projektpfad=projektpfad1,UNSC=T){lines_node<-readLines(paste0(projektpfad,"Obs_Node.out"))
...
atmos.in(0.0026,180,4000,
         projektpfad = projektpfad2)
profile.in(projektpfad = projektpfad2)
hydrus.exe(sleep = 8,file = "undisturbed2",UNSC=F)

sub<-read_hydrus.out(treat=17,projektpfad = projektpfad2,UNSC=F)[[1]]
